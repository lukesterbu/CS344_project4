...
int main(int argc, char *argv[])
{
	...
	while(1) { // Infinite loop
		...
		switch (spawnPid) {
			...
			// Child process
			case 0:
				// Check if we're connected to otp_enc
				if (checkConnection(establishedConnectionFD) == 0) exit(2);
				
				// Get the length of plaintext from the client
				charsRead = recv(establishedConnectionFD, &plainLength, sizeof(plainLength), 0);
				if (charsRead < 0) error("ERROR reading from socket");
				
				plaintext = malloc(sizeof(char) * (plainLength + 1)); // allocate plaintext
				memset(plaintext, '\0', plainLength); // clear plaintext
				int totalRead = 0; // Reset for loop
				// Will receive plaintext in chunks if necessary
				while (totalRead <= plainLength) {
					...
					if (charsRead < 0) error("ERROR reading from socket");
					strncat(plaintext, buffer, charsRead); // concat string
				...
				while (totalWritten <= plainLength) {
					...
					strncpy(buffer, &ciphertext[totalWritten], MAX_SIZE - 1);
					// Send to client
