...
int main(int argc, char *argv[])
{
	...
	ciphertext = readFile(&cipherLength, argv[1]);
	
	// Get the key from the key file
	key = readFile(&keyLength, argv[2]);
	
	// Check if ciphertext is longer than key
	if (cipherLength > keyLength) {
		fprintf(stderr, "Error: key \'%s\' is too short\n", argv[2]);
		exit(1);
	}
	
	// Send the ciphertext length to the server
	charsWritten = send(socketFD, &cipherLength, sizeof(cipherLength), 0);
	if (charsWritten < 0) error("CLIENT: ERROR writing to socket");
	int totalWritten = 0; // reset for loop		
	// Will send ciphertext in chunks if necessary
	...
	printf("%s\n", plaintext); // print out plaintext to stdout
...
// Returns 1 if connections is with otp_dec_d and 0 otherwise
int checkConnection(int connection) {
	...
	// Check if connection is to otp_dec
